...
function defaultBackground(){return{mode:"solid",solid:"#ffffff",grad1:"#ffffff",grad2:"#e8e2da",angle:135,imageId:null,size:"cover",brightness:100,effect:"none",effectOpacity:18,decorations:[],dividerSize:16,dividerMargin:18,paperBorderStyle:"none",paperBorderColor:"#d8d2c8",paperBorderWidth:1,paperInnerInset:0,paperFrameStyle:"none",paperFrameColor:"#bdb5a8",paperShadow:16,paperBorderOpacity:100,paperFrameOpacity:100,effectColor:"#8f887f",effectAutoColor:false,paperBorderAutoColor:false,paperFrameAutoColor:false}}
...
function removeHighlightMarksInSelection(){
  ...
  // Extracting the selected range automatically splits partially selected
  // inline highlight wrappers, leaving the unselected parts in place.
  const frag=range.extractContents();
  ...
  // Deepest first so nested highlight wrappers are safe to unwrap.
  nodes.reverse().forEach(unwrapHighlight);

  // If the extracted fragment itself has top-level MARK wrappers, remove them too.
  [...frag.childNodes].forEach(node=>{
    if(node.nodeType===Node.ELEMENT_NODE && node.matches?.("mark.text-highlight")){
