...
function defaultBackground(){return{mode:"solid",solid:"#ffffff",grad1:"#ffffff",grad2:"#e8e2da",angle:135,imageId:null,size:"cover",brightness:100,effect:"none",effectOpacity:18,effectColor:"#8f887f",decorations:[],dividerSize:16,dividerMargin:18,paperBorderStyle:"none",paperBorderColor:"#d8d2c8",paperBorderWidth:1,paperInnerInset:0,paperFrameStyle:"none",paperFrameColor:"#bdb5a8",paperShadow:16}}
...
function handleEditorInput(){
  ...
  reflowTimer=setTimeout(()=>reflowAllAutoPagesFromCurrentSlot(),120);
}
...
$("applyHighlightBtn").onclick=()=>{
  ...
  applyHighlightToSelection($("highlightColor").value);
};
...
function selectedTextNodeSlices(){
  restoreSelection();
  const sel=getSelection();
...
function wrapTextSlice(node,start,end,className,styleMap){
  node.splitText(end);
...
function applyHighlightToSelection(color){
  const slices=selectedTextNodeSlices();
  if(!slices.length) return false;
  const made=[];
  for(let i=slices.length-1;i>=0;i--){
    const {node,start,end}=slices[i];
    made.push(wrapTextSlice(node,start,end,"text-highlight",{backgroundColor:color}));
  }
  const sel=getSelection(),range=document.createRange();
  range.setStartBefore(made[made.length-1]);range.setEndAfter(made[0]);
...
function clearHighlightFromSelection(){
  ...
  for(let i=slices.length-1;i>=0;i--){
    const {node,start,end}=slices[i];
    made.push(wrapTextSlice(node,start,end,"text-highlight",{backgroundColor:"transparent"}));
  }
  const sel=getSelection(),range=document.createRange();
  range.setStartBefore(made[made.length-1]);range.setEndAfter(made[0]);
  sel.removeAllRanges();sel.addRange(range);savedRange=range.cloneRange();
...
function syncControls(){
  const _deco=current().decorations||current().decoration||{};
  if($("borderOpacity")){$("borderOpacity").value=_deco.borderOpacity??100;$("borderOpacityOut").textContent=(_deco.borderOpacity??100)+"%";}
  if($("innerFrameOpacity")){$("innerFrameOpacity").value=_deco.innerFrameOpacity??100;$("innerFrameOpacityOut").textContent=(_deco.innerFrameOpacity??100)+"%";}
  ...
  $("breakSpacing").value=t.breakSpacing??0;
...
$("effectOpacity").oninput=e=>{current().background.effectOpacity=Number(e.target.value);
...
function applyBackground(){
  ...
  document.documentElement.style.setProperty("--paper-effect-color", b.effectColor||"#8f887f");
  renderDecorations();
}
function renderDecorations(){
  const deco=current().decorations||current().decoration||{};
  if(deco.borderOpacity==null) deco.borderOpacity=100;
  if(deco.innerFrameOpacity==null) deco.innerFrameOpacity=100;
  document.documentElement.style.setProperty("--doc-border-opacity", String(deco.borderOpacity/100));
  document.documentElement.style.setProperty("--doc-inner-opacity", String(deco.innerFrameOpacity/100));
...
$("borderOpacity").oninput=e=>{
  const d=current().decorations||current().decoration||(current().decorations={});
  d.borderOpacity=Number(e.target.value);
  $("borderOpacityOut").textContent=d.borderOpacity+"%";
  persist();renderDecorations();
};
$("innerFrameOpacity").oninput=e=>{
  const d=current().decorations||current().decoration||(current().decorations={});
  d.innerFrameOpacity=Number(e.target.value);
  $("innerFrameOpacityOut").textContent=d.innerFrameOpacity+"%";
  persist();renderDecorations();
};
...
function clonePageForIndex(pageIndex,{forExport=false}={}){
  ...
  ed.style.setProperty("max-height","none","important");
  ed.style.setProperty("contain","paint","important");
  ed.style.setProperty("clip-path","inset(0)","important");
...
function fitPreviewPage(){
  ...
  const availableW=Math.max(220,host.clientWidth-52);
  const availableH=Math.max(300,host.clientHeight-52);
  const naturalW=parseFloat(paper.style.width)||600;
  const naturalH=parseFloat(paper.style.height)||850;
  previewFitScale=Math.min(availableW/naturalW,availableH/naturalH,1)*0.92;
  applyPreviewZoom();
}
...
async function capture(pageIndex=current().currentPageIndex||0){
  ...
  const clone=clonePageForIndex(pageIndex,{forExport:true});host.appendChild(clone);
  requestAnimationFrame(()=>{
    const paper=$("previewHost").querySelector(".paper");
    const ed=paper?.querySelector(".editor");
    const footer=paper?.querySelector(".paper-footer");
    if(paper&&ed&&footer){
      const pr=paper.getBoundingClientRect(), er=ed.getBoundingClientRect(), fr=footer.getBoundingClientRect();
      const scaleNow=pr.width/(parseFloat(paper.style.width)||pr.width)||1;
      const available=Math.max(0,(fr.top-er.top)/scaleNow-8);
      ed.style.setProperty("height",available+"px","important");
      ed.style.setProperty("max-height",available+"px","important");
      ed.style.setProperty("overflow","hidden","important");
    }
    fitPreviewPage();
