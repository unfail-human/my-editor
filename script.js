...
function defaultBackground(){return{mode:"solid",solid:"#ffffff",grad1:"#ffffff",grad2:"#e8e2da",angle:135,imageId:null,size:"cover",brightness:100,effect:"none",effectOpacity:18,decorations:[],dividerSize:16,dividerMargin:18,paperBorderStyle:"none",paperBorderColor:"#d8d2c8",paperBorderWidth:1,paperInnerInset:0,paperFrameStyle:"none",paperFrameColor:"#bdb5a8",paperShadow:16,paperBorderOpacity:100,paperFrameOpacity:100,effectColor:"#8f887f"}}
...
function removeHighlightMarksInSelection(){
  ...
  const candidates=[
    ...$("editor").querySelectorAll("mark.text-highlight,span.text-highlight,span[style*='background-color']")
  ];
  const touched=candidates.filter(el=>{
  ...
  touched.forEach(el=>{
    ...
    if(el.tagName==="MARK"){
      unwrapElement(el);
    }else if(!(el.getAttribute("style")||"").trim() && !el.className){
      unwrapElement(el);
    }
  });
  ...
  while(ancestor && ancestor!==$("editor")){
    if(ancestor.matches?.("mark.text-highlight,span.text-highlight") || ancestor.style?.backgroundColor){
      ancestor.style.backgroundColor="";
      ancestor.style.background="";
      ancestor.classList.remove("text-highlight");
      if(ancestor.tagName==="MARK")unwrapElement(ancestor);
      break;
    }
...
$("effectColor").oninput=e=>{current().background.effectColor=e.target.value;persist();applyBackground()};
...
function fitPreviewPage(){
  ...
  const availableW=Math.max(220,host.clientWidth-72);
  const availableH=Math.max(300,host.clientHeight-72);
  ...
  previewFitScale=Math.min(availableW/naturalW,availableH/naturalH,1)*0.90;
  applyPreviewZoom();
}
