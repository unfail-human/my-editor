...
function dataURLToFile(dataUrl,name,type){
  ...
  return new File([bytes],name||"asset",{type:mime});
}

async function buildSettingsPack(){
  saveCurrent(false);
  const s=current();
  const p=currentPage();
  const pack={
    type:"my-editor-settings",
    version:1,
    exportedAt:new Date().toISOString(),
    typography:structuredClone(s.typography||defaultTypography()),
    background:structuredClone(s.background||defaultBackground()),
    pageStyle:{
      pagePrefix:p.pagePrefix??"—",
      pageSuffix:p.pageSuffix??"—"
    },
    assets:{font:null,background:null}
  };

  // Carry the currently selected custom font with the settings file.
  const family=pack.typography.fontFamily||"";
  if(family.startsWith("user-font-")){
    const id=family.slice("user-".length);
    const asset=await getAsset(FONT_STORE,id);
    if(asset?.file){
      pack.assets.font={
        originalId:id,
        name:asset.name||asset.file.name||"custom-font",
        fileName:asset.file.name||"custom-font",
  ...
  // Carry the active background image too.
  if(pack.background.imageId){
    const asset=await getAsset(BG_STORE,pack.background.imageId);
    if(asset?.file){
      pack.assets.background={
        originalId:pack.background.imageId,
        name:asset.name||asset.file.name||"background",
        fileName:asset.file.name||"background",
        type:asset.file.type||"image/*",
        dataUrl:await fileToDataURL(asset.file)
      };
    }
  }
...
async function exportEditorSettings(){
  try{
    const pack=await buildSettingsPack();
    download(
      new Blob([JSON.stringify(pack,null,2)],{type:"application/json"}),
      "my-editor-settings.json"
    );
  }catch(err){
    console.error(err);
    alert("설정 파일을 만드는 중 오류가 발생했습니다.");
  }
}

async function importEditorSettingsFile(file){
  const text=await file.text();
  const pack=JSON.parse(text);
  if(pack?.type!=="my-editor-settings" || !pack.typography || !pack.background){
    throw new Error("invalid settings pack");
  }

  const s=current();
  const typography={...defaultTypography(),...pack.typography};
  const background={...defaultBackground(),...pack.background};

  // Restore embedded custom font with a fresh local ID.
  if(pack.assets?.font?.dataUrl){
    const fontId="font-"+Date.now();
    const fontFile=dataURLToFile(
      pack.assets.font.dataUrl,
      pack.assets.font.fileName||pack.assets.font.name||"custom-font",
      pack.assets.font.type
    );
    await putAsset(FONT_STORE,{id:fontId,file:fontFile,name:pack.assets.font.name||"사용자 폰트"});
    typography.fontFamily="user-"+fontId;
  }

  // Restore embedded background image with a fresh local ID.
  if(pack.assets?.background?.dataUrl){
    const bgId="bg-"+(Date.now()+1);
    const bgFile=dataURLToFile(
      pack.assets.background.dataUrl,
      pack.assets.background.fileName||pack.assets.background.name||"background",
      pack.assets.background.type
    );
    await putAsset(BG_STORE,{id:bgId,file:bgFile,name:pack.assets.background.name||"배경 이미지"});
    background.imageId=bgId;
  }else if(background.mode==="image"){
    // Missing image asset should not leave a broken image background.
  ...
  // Imported settings are slot-wide. Remove page overrides so every page follows them.
  s.pages.forEach((page,i)=>{
    page.pageTypography={};
    if(pack.pageStyle){
      page.pagePrefix=pack.pageStyle.pagePrefix??page.pagePrefix??"—";
      page.pageSuffix=pack.pageStyle.pageSuffix??page.pageSuffix??"—";
    }
    if(!page.pageNumber || /^\d+$/.test(String(page.pageNumber))){
      page.pageNumber=String(i+1).padStart(2,"0");
    }
  });
  ...
  await loadFonts();
  renderAll();
  syncBackgroundOptionPanels();
  requestAnimationFrame(reflowAllAutoPagesFromCurrentSlot);
}

$("exportSettingsBtn").onclick=exportEditorSettings;
$("importSettingsInput").onchange=async e=>{
  const f=e.target.files?.[0];
  if(!f)return;
  try{
    await importEditorSettingsFile(f);
    alert("설정을 불러왔습니다.");
  }catch(err){
    console.error(err);
    alert("올바른 MY EDITOR 설정 파일이 아닙니다.");
  }finally{
    e.target.value="";
  }
};
