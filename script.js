...
function reflowAllAutoPagesFromCurrentSlot(){
  ...
  try{
    ...
    // First, push overflow forward page by page.
    ...
    // Then pull content backward from auto-generated pages wherever it fits.
    for(let i=0;i<s.pages.length-1;i++){
      ...
      while(changed && next.content){
        ...
        first.remove();
        s.pages[i].content=$("editor").innerHTML;
        next.content=box.innerHTML;
        changed=true;
      }

      if(next.autoGenerated && !next.title && !next.subtitle && !next.content.trim()){
    ...
    renumberPages();
    s.currentPageIndex=Math.min(s.currentPageIndex,s.pages.length-1);
    persist();
    renderAll();
  }finally{
    isPaginating=false;
...
function clearHighlightFromSelection(){
  ...
  const cleanNode=(node)=>{
    if(node.nodeType===Node.ELEMENT_NODE){
      ...
      if(node.tagName==="SPAN" && !node.getAttribute("style")?.trim() && !node.attributes.length){
  ...
  caret.setStart(range.endContainer, Math.min(range.endOffset, range.endContainer.length||0));
  caret.collapse(true);
  sel.addRange(caret);
...
function fitPreviewPage(){
  ...
  const availableW=Math.max(220,host.clientWidth-40);
  const availableH=Math.max(300,host.clientHeight-40);
  const naturalW=parseFloat(paper.style.width)||600;
  const naturalH=parseFloat(paper.style.height)||850;
  const scale=Math.min((availableW/naturalW)*0.94,(availableH/naturalH)*0.94,0.94);
